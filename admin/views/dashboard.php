...
// Variables available: $is_connected, $decay_threshold, $posts_data, $current_tab
?>
<div class="wrap dcd-dashboard">
    ...
    <?php else : ?>
        ...
        <div class="dcd-summary-cards">
            <?php
            $plugin = \DragonContentDecay\Plugin::get_instance();
        ...
        <div class="dcd-sync-status">
            <?php
            $scheduler = new \DragonContentDecay\Scheduler( $analyzer );
            $last_sync = $scheduler->get_last_sync_info();
            ?>
                ...
                <tbody>
                    ...
                    <?php else : ?>
                        <?php foreach ( $posts_data as $post ) : ?>
                            <tr data-trend="<?php echo esc_attr( $post['trend'] ); ?>">
                                ...
                                <td class="column-decay">
                                    <span class="dcd-score dcd-<?php echo esc_attr( $post['trend'] ); ?>">
                                        <?php echo esc_html( number_format( $post['decay_score'], 1 ) ); ?>%
                                    </span>
                                </td>
                                <td class="column-views">
                                    <?php echo esc_html( number_format( $post['pageviews_current'] ) ); ?>
                                </td>
                                <td class="column-previous">
                                    <?php echo esc_html( number_format( $post['pageviews_previous'] ) ); ?>
                                </td>
                                <td class="column-trend">
                                    ...
                                    ?>
                                    <span class="dcd-trend dcd-trend-<?php echo esc_attr( $post['trend'] ); ?>">
                                        <span class="dashicons dashicons-<?php echo esc_attr( $trend_icons[ $post['trend'] ] ); ?>"></span>
                                        <?php echo esc_html( $trend_labels[ $post['trend'] ] ); ?>
                                    </span>
                                </td>
                                <td class="column-updated">
                                    <?php echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $post['post_modified'] ) ) ); ?>
                                </td>
